...
SELECT ff.company_id,
  ...
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 1 THEN ff.amount
      END
    ),
    0
  ) AS revenue_lkr,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 2 THEN ff.amount
      END
    ),
    0
  ) AS gp,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 3 THEN ff.amount
      END
    ),
    0
  ) AS gp_margin,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 4 THEN ff.amount
      END
    ),
    0
  ) AS other_income,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 5 THEN ff.amount
      END
    ),
    0
  ) AS personal_exp,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 6 THEN ff.amount
      END
    ),
    0
  ) AS admin_exp,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 7 THEN ff.amount
      END
    ),
    0
  ) AS selling_exp,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 8 THEN ff.amount
      END
    ),
    0
  ) AS finance_exp,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 9 THEN ff.amount
      END
    ),
    0
  ) AS depreciation,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 10 THEN ff.amount
      END
    ),
    0
  ) AS total_overhead,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 11 THEN ff.amount
      END
    ),
    0
  ) AS provisions,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 12 THEN ff.amount
      END
    ),
    0
  ) AS exchange_gl,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 13 THEN ff.amount
      END
    ),
    0
  ) AS pbt_before_non_ops,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 14 THEN ff.amount
      END
    ),
    0
  ) AS pbt_after_non_ops,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 15 THEN ff.amount
      END
    ),
    0
  ) AS non_ops_exp,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 16 THEN ff.amount
      END
    ),
    0
  ) AS non_ops_income,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 17 THEN ff.amount
      END
    ),
    0
  ) AS np_margin,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 18 THEN ff.amount
      END
    ),
    0
  ) AS ebit,
  COALESCE(
    MAX(
      CASE
        WHEN ff.metric_id = 19 THEN ff.amount
      END
    ),
    0
  ) AS ebitda,
...
SELECT *
FROM analytics.financial_monthly_view;
CREATE OR REPLACE VIEW analytics.vw_financial_pnl AS WITH actuals AS (
    SELECT *
    FROM analytics.financial_monthly_view
...
SELECT COALESCE(a.company_id, b.company_id) AS company_id,
  ...
  1.0::numeric AS exchange_rate
FROM actuals a
  FULL OUTER JOIN budgets b ON a.company_id = b.company_id
