      ...
      <div className="max-w-[1400px] mx-auto p-6">
            ...
            {riskRadar.map((item) => (
              <div
                ...
                className={`p-4 rounded-lg border-2 ${
                  item.riskScore >= 80 ? "border-red-300 bg-red-50" :
                  item.riskScore >= 50 ? "border-amber-300 bg-amber-50" :
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-2xl font-bold ${
                    ...
                    item.riskScore >= 50 ? "text-amber-600" :
                    "text-emerald-600"
                  }`}>
                    {item.riskScore}
                  </span>
        ...
        <div className="space-y-4">
          {filteredVariances.map((item) => (
            <div key={item.id} className={`bg-white rounded-lg border-l-4 border p-5 ${
              item.severity === "critical" ? "border-l-red-500 border-red-200" :
              item.severity === "high" ? "border-l-amber-500 border-amber-200" :
              item.severity === "medium" ? "border-l-blue-500 border-blue-200" :
