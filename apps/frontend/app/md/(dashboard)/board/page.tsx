...
import { Download, Lock, MessageSquare, Calendar, FileSpreadsheet, History } from "lucide-react";

// Board report data (will be fetched from API)
const boardReports = [
  { id: "1", period: "October 2025", year: 2025, month: 10, status: "draft", lastUpdated: "Dec 23, 2025", author: "CEO", locked: false },
  { id: "2", period: "September 2025", year: 2025, month: 9, status: "approved", lastUpdated: "Nov 15, 2025", author: "CEO", locked: true },
  { id: "3", period: "August 2025", year: 2025, month: 8, status: "approved", lastUpdated: "Oct 12, 2025", author: "CEO", locked: true },
  { id: "4", period: "June 2025", year: 2025, month: 6, status: "approved", lastUpdated: "Jul 20, 2025", author: "CEO", locked: true },
  { id: "5", period: "May 2025", year: 2025, month: 5, status: "approved", lastUpdated: "Jun 15, 2025", author: "CEO", locked: true },
  { id: "6", period: "April 2025", year: 2025, month: 4, status: "approved", lastUpdated: "May 12, 2025", author: "CEO", locked: true },
];
...
// Cluster summary data (will be from API)
const clusterSummary = [
  ...
  { name: "Lube 02", month: -15000, ytd: -150000, ach: -103.4, status: "critical" },
  { name: "Bunkering", month: -45000, ytd: -450000, ach: -225, status: "critical" },
  { name: "Ship Supply", month: 73100, ytd: 731000, ach: 97.5, status: "on-track" },
  { name: "Property", month: 47800, ytd: 478000, ach: 103.9, status: "on-track" },
  { name: "Warehouse", month: 44300, ytd: 443000, ach: 110.8, status: "exceeding" },
  { name: "Manufacturing", month: 40800, ytd: 408000, ach: 108.8, status: "exceeding" },
  { name: "Hotel & Leisure", month: 8500, ytd: 85000, ach: 85.0, status: "warning" },
  { name: "Strategic Inv.", month: -5200, ytd: -52000, ach: -173.3, status: "critical" },
];
...
export default function BoardViewPage() {
  ...
  // Export Excel function - downloads Group Financial Summary P&L Template
  const handleExportExcel = async () => {
    setIsExporting(true);
    try {
      // API call to generate Excel with filled data
      const response = await fetch(`/api/export/financial-summary?year=${selectedReport.year}&month=${selectedReport.month}`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `Group_Financial_Summary_${selectedReport.period.replace(' ', '_')}.xlsx`;
        document.body.appendChild(a);
        a.click();
        window.URL.revokeObjectURL(url);
        document.body.removeChild(a);
      } else {
        // Fallback: download CSV with current data
        downloadCSVFallback();
      }
    } catch (error) {
  ...
  // Fallback CSV download with real data
  const downloadCSVFallback = () => {
    const headers = ['Cluster', 'Month PBT (LKR)', 'YTD PBT (LKR)', 'Achievement %', 'Status'];
    const rows = clusterSummary.map(row => [
      row.name,
      row.month,
      row.ytd,
      `${row.ach}%`,
    ...
    const csvContent = [
      `Group Financial Summary - ${selectedReport.period}`,
      '',
      headers.join(','),
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `Group_Financial_Summary_${selectedReport.period.replace(' ', '_')}.csv`;
    document.body.appendChild(a);
    a.click();
    window.URL.revokeObjectURL(url);
    document.body.removeChild(a);
      ...
      <div className="max-w-[1400px] mx-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="flex items-center gap-2 h-10 px-4 text-sm font-medium text-white bg-[#0b1f3a] rounded-lg hover:bg-[#0b1f3a]/90 disabled:opacity-50"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-semibold text-slate-900">Cluster Performance Summary</h3>
                <button
                  onClick={downloadCSVFallback}
                  className="flex items-center gap-1 text-xs text-[#0b1f3a] hover:underline"
                >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tfoot className="bg-slate-100 border-t-2 border-slate-300">
                    <tr>
                      <td className="px-4 py-3 font-bold text-slate-900">Group Total</td>
                      <td className="px-4 py-3 text-right font-bold text-slate-900">
                        {clusterSummary.reduce((sum, r) => sum + r.month, 0).toLocaleString()}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-slate-900">
                        {clusterSummary.reduce((sum, r) => sum + r.ytd, 0).toLocaleString()}
                      </td>
                      <td className="px-4 py-3 text-right font-bold text-[#0b1f3a]">91.8%</td>
                      <td className="px-4 py-3 text-center">
                        <span className="px-2 py-1 text-xs font-medium rounded bg-blue-100 text-blue-700">on-track</span>
                      </td>
                    </tr>
                  </tfoot>
                </table>
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-lg border border-slate-200 p-5">
              <div className="flex items-center gap-2 mb-4">
                <History className="h-5 w-5 text-[#0b1f3a]" />
                <h3 className="text-base font-semibold text-slate-900">Past Reports</h3>
              </div>
              <div className="space-y-3">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-5">
              ...
              <div className="space-y-2">
                <button 
                  onClick={handleExportExcel}
                  disabled={isExporting}
                  className="w-full flex items-center gap-2 h-10 px-4 text-sm font-medium text-white bg-[#0b1f3a] rounded-lg hover:bg-[#0b1f3a]/90 disabled:opacity-50"
                >
                  <FileSpreadsheet className="h-4 w-4" />
                  {isExporting ? 'Exporting...' : 'Download P&L Template'}
                </button>
